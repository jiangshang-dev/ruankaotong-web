...
export interface EssayThinkStep {
  id: string
  title: string
  content: string
  status: 'running' | 'done'
...
const steps = computed((): EssayThinkStep[] => splitThinkingSteps(props.thinking, props.loading))
const visible = computed(() => steps.value.length > 0 || props.loading)

const headerTitle = computed(() => {
  if (!props.loading) return '已完成思考'
  const running = [...steps.value].reverse().find((s) => s.status === 'running')
  return running?.title || '正在思考中'
})
...
function splitThinkingSteps(text: string, running: boolean): EssayThinkStep[] {
  const raw = String(text || '')
    .replace(/<\/?think>/gi, '')
    .trim()
  if (!raw) return []
  let chunks = raw.split(/(?=^#{1,3}\s+\S)/m).map((s) => s.trim()).filter(Boolean)
  if (chunks.length < 2) {
    chunks = raw
      .split(/\n(?=(?:第[一二三四五六七八九十\d]+步|\d+[\.、])\s)/)
      .map((s) => s.trim())
      .filter(Boolean)
  }
  if (chunks.length < 2) {
  ...
  return chunks.map((chunk, i) => {
    ...
    const first = lines[0] || `步骤 ${i + 1}`
    const rest = /^#{1,3}\s+/.test(first) ? lines.slice(1).join('\n').trim() : chunk
    return {
      id: `think-${i}`,
      title: summarize(first),
      content: rest || chunk,
      status: (running && i === chunks.length - 1 ? 'running' : 'done') as EssayThinkStep['status'],
    }
  })
}
</script>
...
<template>
  <div
    v-if="visible"
    class="think-box"
    :class="{ collapsed, loading }"
  >
    <button class="think-head" type="button" @click="collapsed = !collapsed">
      <span class="think-head-left">
        <span v-if="loading" class="think-spinner" />
        <svg
          v-else
    ...
    <div v-show="!collapsed" class="think-body">
      ...
      >
        <div class="think-rail">
          <span class="think-dot" :class="step.status">
            <template v-if="step.status === 'done'">✓</template>
          </span>
          <span v-if="index < steps.length - 1" class="think-line" />
        </div>
        <div class="think-main">
          <div class="think-title">{{ step.title }}</div>
          <div class="think-md" v-html="markdownToHtml(step.content)" />
        </div>
      </div>
      <div v-if="!steps.length && loading" class="think-empty">思考进行中…</div>
    </div>
  </div>
</template>
